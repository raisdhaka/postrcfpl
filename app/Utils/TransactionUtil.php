...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'rp_earned' => $input['status'] == 'final' ? $this->calculateRewardPoints($business_id, $final_total, $input['products']) : 0,
    ...
    */
     ...
     * @return integer
     */
    public function calculateRewardPoints($business_id, $total, $products = null)
    {
        ...
        if ($business->enable_rp == 1) {
            ...
            foreach($products as $value){
                $product = Variation::where('product_id', '=', $value['product_id'])->first();
                $product_count = $value['quantity'];
                $sell_price = $product->default_sell_price;
                $purchase_price = $product->default_purchase_price;
                $price = $sell_price - $purchase_price;
                $total_profit = $total_profit + ($price*$product_count);
            }
            $total_points = $total_profit/$amount_per_unit_point;
